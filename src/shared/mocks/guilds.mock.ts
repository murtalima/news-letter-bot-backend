...
export const GuildsMockRepository = jest.fn(() => ({
    findOne: jest.fn((query: { where: { discordId: string }}) => {
        const result: Guild = guildMockDb.find((e) => e.discordId == query.where.discordId)
        return result;
    }),
    create: jest.fn((dto:CreateGuildDto) => {
        guildMockDb.push({ ...dto, createdAt: new Date(), updatedAt: new Date(), id: guildMockDb[guildMockDb.length-1].id + 1} as Guild)
