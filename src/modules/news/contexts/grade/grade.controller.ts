...
import { ApiBadRequestResponse, ApiConsumes, ApiNotFoundResponse, ApiOkResponse, ApiOperation, ApiParam, ApiQuery, ApiTags } from "@nestjs/swagger";
...
export class GradeNewspaperController {
  ...
  execute(@Param('newspaperId') newspaperId: number, @Param('userId') userId: string, @Query('grade') grade: string) {
