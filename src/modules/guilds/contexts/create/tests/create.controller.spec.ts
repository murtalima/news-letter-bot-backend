  ...
  describe("Create", () => {
    ...
    it("should throw a guild already created", async () => {
     const existentGuild= guildMockDb[0]
      ...
      await expect(createGuildsController.execute(dto)).rejects.toThrow( new BadRequestException(alreadyExist('guild')))
    ...
    it("should create a guild successfully", async () => {
      ...
      await expect(createGuildsController.execute(dto)).resolves.toEqual(expect.objectContaining({
        id: 4,
